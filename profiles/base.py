...
A Profile encapsulates everything that's scenario-specific:
  ...
  - Hooks for agent behavior (loop detection, time budget, etc.)
...
class AgentConfig:
    ...
    enabled: bool = True
    hooks: list = field(default_factory=list)  # list[AgentHook]
    time_budget: float | None = None  # seconds; None = no limit
    # MCP bridges (BrowserMcpClient, future servers, …). Each entry should
    # satisfy browser_mcp_client.McpBridge: connect / list_openai_schemas /
...
class ProfileConfig:
    ...
    This lets you adjust thresholds, time budgets, and hook settings
