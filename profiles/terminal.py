...
- Project facts live in project_memory.json and appear as [PROJECT MEMORY] in context. \
Read that file only if you need full detail beyond the summary.
- If the user states a lasting preference, call remember_preference(key, value).
...
- RETHINK: stop repeating the same approach; wait for the next turn's automatic \
state refresh, then change strategy before trying again.
